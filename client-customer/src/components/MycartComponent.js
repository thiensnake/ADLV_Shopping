...
class Mycart extends Component {
  ...
  createOrder={(data, actions) => {
    return actions.order.create({
      purchase_units: [
        {
          amount:{
            value: 200, // Giá tiền cần thanh toán 
          },
        },
      ],
    });
  ...
  onApprove={function (data, actions) {
    // Hành động khi người dùng approve thanh toán
  }}
/>  
</PayPalScriptProvider>
</div>
  ...
  // Updated API Checkout method with dynamic toast notifications
  apiCheckout(total, items, customer) {
    ...
    axios.post('/api/customer/checkout', body, config)
      ...
      .catch((error) => {
        console.error('Error during API checkout:', error);
