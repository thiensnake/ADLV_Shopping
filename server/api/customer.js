...
router.put('/customers/:id', JwtUtil.checkToken, async function (req, res) {
  ...
  const customer = { _id: _id, username: username, password: password, name: name, phone: phone, email: email, diachi: diachi };
...
router.post('/login', async function (req, res) {
  ...
  if (!username && !password) {
    ...
    res.json({ success: false, message: 'Vui lòng nhập tên đăng nhập.' });
  } else if (!password) {
    res.json({ success: false, message: 'Vui lòng nhập mật khẩu.' });
  } else {
...
router.post('/signup', async function (req, res) {
  ...
  if (dbCust) {
    ...
    const newCust = { username: username, password: password, name: name, phone: phone, email: email, active: 0, token: token, diachi: '' };
    ...
    if (result) {
      ...
      res.json({ success: false, message: 'Lỗi dữ liệu' });
    }
  }
});
